...
name: humanizer-zh
description: |
  当用户要求最终交付内容主要为中文时，编辑或审阅中文文本，去除 AI 写作痕迹，
  同时保留事实、含义、语气和作者声音。仅用于中文输出，也可只处理双语文档中的
  中文部分。最终输出主要为英文时不得使用，应改用 humanizer-en。如果同一改写任务
  显式调用 humanizer-zh 和 humanizer-en，不得立即改写；先且只询问一次：两个 skill
  分别处理对应语言部分、仅使用 humanizer-zh，还是仅使用 humanizer-en，并等待回答。
  两个 skill 不得处理同一段落。其他情况下若目标语言有实质性歧义，先询问最终语言。
---
...
仅当用户要求的最终输出主要为中文时继续。双语文档只处理其中的中文部分。如果最终输出主要为英文，停止并改用 `humanizer-en`。不得让两个 humanizer skill 重写同一段落。

### 同时调用时的共享确认

如果同一个改写请求显式调用 `humanizer-zh` 和 `humanizer-en`，在分析或改写文本前暂停。使用用户当前的语言只询问一次：两个 skill 分别处理对应语言部分、仅使用 `humanizer-zh`，还是仅使用 `humanizer-en`？等待用户回答。不得把同时显式调用视为已经同意两个都用，也不得让两个 skill 分别重复提问。

如果用户选择两个都用，`humanizer-zh` 只处理双语文档的中文部分，`humanizer-en` 只处理英文部分；如果用户只选择一个，不得调用另一个。不得把同一段落依次交给两个 skill。
