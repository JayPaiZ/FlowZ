...
name: humanizer-en
description: |
  Rewrite or review prose when the requested final output is primarily English,
  removing AI-writing patterns while preserving facts, meaning, tone, and voice.
  Use only for English deliverables, including English sections of a bilingual
  document. Never use for a primarily Chinese final output; use humanizer-zh
  instead. If both humanizer-en and humanizer-zh are explicitly invoked for one
  rewriting task, do not rewrite yet. Ask once whether to use both on separate
  language sections, only humanizer-en, or only humanizer-zh, then wait for the
  answer. Never apply both skills to the same passage. If the target language is
  otherwise materially ambiguous, ask which final language the user wants.
---

# Humanizer: remove AI writing patterns

## Language routing guard

Proceed only when the requested final output is primarily English. For a bilingual document, edit only its English sections. If the requested final output is primarily Chinese, stop and use `humanizer-zh` instead. Never run both humanizer skills over the same passage.

### Shared confirmation when both skills are invoked

When one rewriting request explicitly invokes both `humanizer-en` and `humanizer-zh`, stop before analyzing or rewriting the text. Ask exactly one shared confirmation in the user's current language: use both skills on their respective language sections, use only `humanizer-en`, or use only `humanizer-zh`? Wait for the user's answer. Do not treat the two explicit invocations as permission to proceed, and do not ask once per skill.

If the user chooses both, apply `humanizer-en` only to English sections and `humanizer-zh` only to Chinese sections of the bilingual document. If the user chooses one, do not apply the other skill. Never pass the same passage through both skills.
