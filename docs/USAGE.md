...
Agent 会自己读取 FlowZ 里的安装资源，安装全局规则和配套 Skill。安装完成后，其他新项目和旧项目也可以使用这套工作流，不需要把 FlowZ 文件夹复制到每个项目里。
...
FlowZ 会尽量减少重复读取、重复提问和无效输出。对于较长文字的解释、整理、比较或审查，如果内部聊天窗口只靠你提供的文字或一个文本文件就能完成，而且不需要读取项目、修改文件或运行命令，Agent 可能会建议你把这部分工作交给内部聊天窗口处理。

这样做是为了减少当前开发任务中的上下文消耗。它不是强制步骤，也不包含图片分析。你可以使用内部聊天窗口，也可以继续在当前 Agent 中完成。
