...
FlowZ 会尽量减少重复读取、重复提问和无效输出。对于较长文字的解释、整理、比较或审查，如果内部AI对话网页只靠你提供的文字或一个文本文件就能完成，而且不需要读取项目、修改文件或运行命令，Agent 可能会建议你把这部分工作交给内部AI对话网页处理。

这样做是为了减少当前开发任务中的上下文消耗。它不是强制步骤。你可以使用内部AI对话网页，也可以继续在当前 Agent 中完成。

省 token 不只靠内部AI对话网页。FlowZ 还会尽量选择最小的任务流程，减少重复读取和重复总结，失败后只携带新增证据，并把能由用户直接完成的部分命令和简单验证方法告诉你。你可以按提示自己运行命令、查看结果，再把结果反馈给 Agent，这同样能减少不必要的对话和调用。
...
用户明确要求时，Agent 应该按照你的要求执行，不应自行坚持建议或停止建议。
...
- 对网页端可以独立完成的较大文字任务，提供一个可选的辅助通道，减少当前 Agent 的重复上下文；
